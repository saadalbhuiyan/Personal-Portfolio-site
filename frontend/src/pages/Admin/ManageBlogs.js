...
const ManageBlogs = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 bg-[var(--bg-light)] rounded-lg shadow-md">
            <h1 className="text-[var(--text-primary)] text-3xl md:text-4xl font-bold mb-8 text-center">Manage Blogs</h1>

            <form onSubmit={handleSubmit} className="mb-10 p-6 bg-[var(--card-bg)] rounded-lg shadow">
                <h2 className="text-xl text-[var(--text-primary)] font-semibold mb-6">{isEditing ? 'Edit Blog' : 'Add New Blog'}</h2>

                <label className="block mb-2 font-medium text-[var(--text-primary)]">Title</label>
                <input
                    type="text"
                    name="title"
                    value={formData.title}
                    onChange={handleChange}
                    className="w-full p-2 mb-5 border border-[var(--border)] rounded focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
                    required
                    aria-required="true"
                />

                <label className="block mb-2 font-medium text-[var(--text-primary)]">Content</label>
                <textarea
                    name="content"
                ...
                <div className="flex items-center gap-4">
                    <button
                        type="submit"
                        className="bg-[var(--primary)] text-[var(--bg-light)] px-6 py-2 rounded hover:bg-[var(--secondary)] transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-[var(--accent)]"
                    >
                    ...
                    {isEditing && (
                        <button
                            ...
                            className="px-5 py-2 border rounded hover:bg-[var(--bg-light)] focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
                        >
                            Cancel
                        </button>
                    )}
            ...
            {loading ? (
                ...
                <table className="w-full border border-[var(--border)] rounded border-collapse">
                    <thead>
                    ...
                    <tbody>
                    {blogs.map((blog) => (
                        <tr key={blog._id} className="border-t border-[var(--border)]">
